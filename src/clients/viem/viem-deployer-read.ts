import { createPublicClient, http, parseEther, formatEther, BaseError, ContractFunctionRevertedError, ContractFunctionName, ContractFunctionArgs } from 'viem';
...
export class ViemDeployerReader {
  ...
  constructor(config: ViemSDKConfig) {
    ...
    this.stateManagerAddress = networkContracts.STATE_MANAGER_ADDRESS as Address;
  }

  private async callDeployer<
    functionName extends ContractFunctionName<typeof DEPLOYER_ABI, 'pure' | 'view'>,
    const args extends ContractFunctionArgs<typeof DEPLOYER_ABI, 'pure' | 'view', functionName>,
  ...
  private async callStateManager<
    functionName extends ContractFunctionName<typeof STATEMANAGER_ABI, 'pure' | 'view'>,
    const args extends ContractFunctionArgs<typeof STATEMANAGER_ABI, 'pure' | 'view', functionName>,
  >(
    functionName: functionName,
    args: args
  ): Promise<any> {
  ...
  async getTokenDeploymentConfig(token: Address): Promise<TokenDeploymentConfig> {
  ...
  async getTokenDeploymentConfigsMapping(token: Address): Promise<TokenDeploymentConfig> {
